...
reference_site_regions_csv = snakemake.input.reference_site_regions
...
# add the regions
reference_site_regions = pd.read_csv(reference_site_regions_csv)
site_map = (
    site_map
    .assign(reference_site=lambda x: x["reference_site"].astype(str))
    .merge(
        reference_site_regions[["reference_site", "region"]].assign(
    ...
    .assign(region=lambda x: x["region"].fillna(method="ffill"))
)
